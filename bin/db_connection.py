...
class dbConnector:
    ...
    def insert_planner(self, planner_type):
        planner = self.client[self.db_name].planner
        planner.insert_one(planner_type)
        # self.client.close()
    
    def insert_if_restart(self, if_restart):
        restart = self.client[self.db_name].restart
        restart.insert_one(if_restart)
