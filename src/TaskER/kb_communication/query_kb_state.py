    ...
    if da_type in executable_tasks:
        if da_type == "clean":
            ...
            for i in range(len(goals)):
                ...
                # basic (not updated with current position) distance Knowledge item
                old_ki = KnowledgeItem()
                old_ki.knowledge_type = KnowledgeItem.FUNCTION
                old_ki.attribute_name = "distance"
                old_ki.values.append(diagnostic_msgs.msg.KeyValue("location", "stpt"))
                old_ki.values.append(diagnostic_msgs.msg.KeyValue("location", goals[i]))

                # INSTEAD, TASK'S KB IS UPDATED DIRECTLY
                remove_from_kb(old_ki, id)
                add_to_kb(ki, id)

                # MODIFIED FOR NON-MONGODB APPROACH
                # NO INSERTION TO DB IS PERFORMED
            ...
            old_ki.attribute_name = "distance"
            old_ki.values.append(diagnostic_msgs.msg.KeyValue("location", "stpt"))
            old_ki.values.append(diagnostic_msgs.msg.KeyValue("location", goal))
...
# 
# 
#       FUNCTIONS BELOW COMMENTED FOR INFORMATIVE REASONS
#       DUE TO MIGRATION TO NON-MONGODB APPROACH
# 
# 
# 
...
#         for pi in permuted_instances:remove_from_kb
#             ki = KnowledgeItem()
#             ki.knowledge_type = KnowledgeItem.FACT
