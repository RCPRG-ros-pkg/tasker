...
class DynAgent(Node):
    def __init__(self, da_name, da_id, da_type, ptf_csp, da_state_name):
        ...
        self.da_suspend_request.setData(
            ["cmd", "", "param_name", "suspension requirements from the task harmoniser"])
        self.is_initialised = False
        self.tasker_communicator = DACommunicator(
            da_name=da_name, cond_cost_handler=self.startConditionHandler, sus_cost_handler=self.suspendConditionHandler)
        ...
        if not self.terminateFlag == True:
            ...
            if self.main_sm.is_running():
                if self.da_state[0] == "Init" or self.getActiveStates(self.main_sm)[0][0] in ["Wait", "UpdateTask"]:
                    self.cmd_handler(
                        CMD(recipient_name=self.name, cmd="terminate"))
                    self.da_state = ["END"]
                    ...
                    print(
                        "DA triggered self termination flag. It is in CMD state, so the flag triggers preemption")
                ...
                print(
                    "DA -> TaskER Termination not required, because TaskER FSM is not running. Sending <END> state to THA")
                self.cmd_handler(
                    CMD(recipient_name=self.name, cmd="terminate"))
                self.da_state = ["END"]
                my_status.da_state = self.da_state
    ...
    def run(self, main_sm, sis=None):
        ...
        if USE_SMACH_INRTOSPECTION_SERVER:
            sis = smach_ros.IntrospectionServer(
                str("/"+self.name+"smach_view_server"), self.main_sm, self.name)
            sis.start()
        else:
            sis = None
        ...
        if not self.terminateFlag:
            ...
            thread_status = threading.Thread(
                target=self.sendStatusThread, args=(1,))
            thread_status.start()
            ...
            thread_cost_cond = threading.Thread(
                target=self.costCondThread, args=(1,))
            thread_cost_cond.start()
            print("started cmd cost_cond thread")
            print("starting cmd susp_cond thread")
            thread_susp_cond = threading.Thread(
                target=self.suspCondThread, args=(1,))
            ...
            smach_thread = threading.Thread(
                target=self.main_sm.execute, args=(smach.UserData(),))
            smach_thread.start()
    ...
    def sendStatusThread(self, args):
        ...
        while not self.terminateFlag:
            if not self.tasker_communicator.get_tha_alive_flag():
                print('DynAgent "' + self.name +
                      '" detected the task_harmonizer is dead')
                self.terminateDA()
